 ...
 * - cleaner lines
 * - cleaner addNode
 * - "words" mode" first word not showm
 * index.html
 * x fixed form
 * x or hamburger popup
 * 
 */
...
(function(window){
            ...
            for(var r = 0; r < this.state.rows; r++) {
                ...
                for(var c = 0; c < this.state.columns; c++) {
                    ...
                    if(column){
                        ...
                        this.nodes.columns.push(column);
                    }
                }
            }
     ...
     * @returns {number} index of affected column or last index of columns when all affected
     */
     
    Sphere.prototype.columNode = function(node, index){
        index = index || -1;
        ...
        var addNode = function(index, node){
            ...
            columns[index].appendChild(wrapper);
            return index;
        };
